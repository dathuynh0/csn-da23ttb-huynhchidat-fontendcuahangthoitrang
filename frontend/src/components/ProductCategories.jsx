...
const Card = ({ title, description, imageUrl, link }) => {
  return (
    <div className="w-full flex flex-col">
      <Link to={link} className="block overflow-hidden aspect-[1/1]">
        <img
        ...
        <div className="mt-6">
          <Link to={link}>
            <Button
              size="lg"
              variant="outline"
              className="text-lg font-light px-10 py-6 hover:bg-black/80 hover:text-white/90 transition-colors duration-300 cursor-pointer"
            >
              Mua ngay
            </Button>
          </Link>
